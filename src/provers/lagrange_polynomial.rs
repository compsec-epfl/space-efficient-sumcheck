...
impl<F: Field> Iterator for LagrangePolynomial<F> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        // Step 2: check if position is 0, which is a special case where we do nothing
        if self.position == 0 {
            self.position = Hypercube::next_gray_code(self.position);
            return Some(self.value);
        }

        // Step 3: check if this iteration yields zero, in which case we skip processing
        ...
        // Step 4: compute which bit_index flipped and in which direction
        let bit_diff = self.last_position ^ self.position;
        assert!(bit_diff.count_ones() == 1); // gray code ordering yields one bit hamming distance
