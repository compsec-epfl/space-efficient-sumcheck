...
/// The witness should be two elements in the field
#[derive(Clone)]
...
impl<F: Field> Witness<F> {
    ...
    pub(crate) fn new(g0: F, g1: F) -> Witness<F> {
        Witness {
            g0: g0,
            g1: g1,
        }
...
impl<F> Prover<F> for BasicProver<F>
where
    F: Field,
{
    ...
    fn fold(&mut self, r: F) {
        ...
        self.f = fold_polynomial(&self.f, r);
...
fn test_trivial_prover() {
    ...
    let f = DensePolynomial::rand(1, rng);
    let g = DensePolynomial::rand(1, rng);
    let witness = Witness::<Fr>::new(&f, &g);
    let mut prover = BasicProver::new(witness);
